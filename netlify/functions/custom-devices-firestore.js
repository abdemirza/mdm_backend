// Firebase Firestore-based Custom Devices API for Netlify Functions
...
class FirestoreService {
  ...
  async getDeviceByField(fieldName, value) {
    try {
      const response = await this.makeFirestoreRequest('GET', `/devices?where=${fieldName}=="${value}"`);
      if (response.documents && response.documents.length > 0) {
        return this.convertFromFirestoreFields(response.documents[0]);
      }
      return null;
    } catch (error) {
  ...
  async getDevicesByStatus(status) {
    try {
      const response = await this.makeFirestoreRequest('GET', `/devices?where=status=="${status}"`);
      if (response.documents) {
        return response.documents.map(doc => this.convertFromFirestoreFields(doc));
      }
      return [];
